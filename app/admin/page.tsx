...
export default function AdminPage() {
  ...
  const openPartnerModal = (partner: Partner) => {
    ...
    setPartnerComment(partner.notes || "");
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {cityMap.length > 0 ? (
                cityMap.map(([city, count]) => (
                  <div key={city} className="bg-indigo-50 rounded-xl p-4 text-center border border-indigo-100 hover:bg-indigo-100 transition">
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {selectedPartner.status !== "Approved" && (
                <button
                  onClick={() => updatePartner(selectedPartner._id, "Approved", partnerComment)}
                  disabled={updatingPartner}
              ...
              {selectedPartner.status !== "Rejected" && (
                <button
                  onClick={() => {
                    ...
                    updatePartner(selectedPartner._id, "Rejected", partnerComment);
                  }}
                  disabled={updatingPartner}
              ...
              {selectedPartner.status !== "Pending" && (
                <button
                  onClick={() => updatePartner(selectedPartner._id, "Pending", partnerComment)}
                  disabled={updatingPartner}
