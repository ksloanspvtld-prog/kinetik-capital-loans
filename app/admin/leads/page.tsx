...
export default function AdminPage() {
  ...
  const exportPartnersToExcel = () => {
    ...
    saveAs(file, `Partners_${new Date().toISOString().split("T")[0]}.xlsx`);
  };
  ...
  return (
    <div className="p-4 md:p-10 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
      ...
      {activeTab === "leads" ? (
        <>
          {/* LEAD TAB CONTENT - तुमचा existing leads content */}
          {/* Export & Total */}
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-8">
            <div className="bg-gradient-to-br from-indigo-400 to-indigo-500 text-white p-4 rounded-2xl shadow-lg">
              <p className="text-xs opacity-90 font-medium">Total Partners</p>
              <h2 className="text-3xl font-bold mt-2">{partnerStats.total}</h2>
            </div>
            <div className="bg-gradient-to-br from-yellow-400 to-yellow-500 text-white p-4 rounded-2xl shadow-lg">
              <p className="text-xs opacity-90 font-medium">Pending</p>
              <h2 className="text-3xl font-bold mt-2">{partnerStats.pending}</h2>
            </div>
            <div className="bg-gradient-to-br from-emerald-400 to-emerald-500 text-white p-4 rounded-2xl shadow-lg">
              <p className="text-xs opacity-90 font-medium">Approved</p>
              <h2 className="text-3xl font-bold mt-2">{partnerStats.approved}</h2>
            </div>
            <div className="bg-gradient-to-br from-rose-400 to-rose-500 text-white p-4 rounded-2xl shadow-lg">
              <p className="text-xs opacity-90 font-medium">Rejected</p>
              <h2 className="text-3xl font-bold mt-2">{partnerStats.rejected}</h2>
            </div>
          </div>

          {/* Location/State Wise DSA Map */}
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50 p-6 mb-8">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">📍 DSA Location Map</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {(() => {
                ...
                return sortedCities.length > 0 ? (
                  sortedCities.map(([city, count]) => (
                    <div key={city} className="bg-indigo-50 rounded-xl p-4 text-center border border-indigo-100">
                      <p className="text-2xl font-bold text-indigo-600">{count}</p>
                      <p className="text-sm text-slate-600 font-medium">{city}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-slate-400 col-span-full text-center">No partners data available</p>
                );
              })()}
            </div>
          </div>
          ...
          <div className="flex flex-wrap justify-between items-center mb-6 gap-3">
            <button
              ...
              className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-xl hover:shadow-lg transition flex items-center gap-2 text-sm"
            >
              📥 <span className="hidden sm:inline">Export Partners to Excel</span>
            </button>
            <span className="text-sm text-slate-400 bg-white/70 px-4 py-2 rounded-xl">
              Total: <span className="font-semibold text-slate-700">{partners.length}</span> partners
            </span>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-slate-800 to-slate-900 text-white">
                  ...
                  <th className="p-4 text-left text-xs font-medium uppercase tracking-wider">Mobile</th>
                  <th className="p-4 text-left text-xs font-medium uppercase tracking-wider">City</th>
                  <th className="p-4 text-left text-xs font-medium uppercase tracking-wider">Type</th>
              ...
              <tbody>
                {partners.length === 0 ? (
                  ...
                  partners.map((partner) => (
                    <tr key={partner._id} className="hover:bg-slate-50 transition border-b border-slate-100">
                      ...
                      <td className="p-4 text-sm text-slate-600">{partner.mobile}</td>
                      <td className="p-4 text-sm text-slate-600">{partner.city || "N/A"}</td>
                      <td className="p-4 text-sm text-slate-600 capitalize">{partner.partnerType || "individual"}</td>
                      <td className="p-4">
